...
detectors.each{ det ->
  ...
  userPIDList.each{ pid ->
    ...
    */
    ...
    def grU,grU_good,grU_bad //NOTE: ADDED 5/17/23
    ...
    sectors.each { s ->
      ...
      ratioTree[sectorIt].each { epochIt,ratioList ->
        ...
        insertEpochPlot(epochPlotTree[sectorIt][epochIt],
          "grU",defineEpochPlot("grU_epoch","Ungated Faraday Cup charge F [nC]",sectorIt,epochIt))//NOTE: ADDED 5/17/23
        insertEpochPlot(epochPlotTree[sectorIt][epochIt],
    ...
    def outHipoU = new TDirectory()//NOTE: ADDED 5/17/23
    def outHipoF = new TDirectory()
    def outHipoFA = new TDirectory()//NOTE: ADDED 5/17/23
    def outHipoLTT = new TDirectory()//NOTE: ADDED 5/17/23
    def outHipoLTA = new TDirectory()//NOTE: ADDED 5/17/23. REMOVED def outHipoT = new TDirectory()
    ...
    def TLFA = defineTimeline("Accumulated Faraday Cup Charge [mC]","Charge","F")//NOTE: ADDED 5/17/23
    def TLTT = defineTimeline("Live Time, from Total Gated FC Charge / Total Ungated FC Charge","Live Time","LT")//NOTE: ADDED 5/17/23
    def TLTA = defineTimeline("Live Time, from Average RUN::scaler::livetime","Live Time","LT")//NOTE: ADDED 5/17/23. REMVOVED: def TLT = defineTimeline("Live Time","Live Time","LT")
    ...
    def addGraphsToHipo = { hipoFile ->
      ...
      writeHipo(
        ...
        [
          ...
          grN_good,grN_bad,
          grF_good,grF_bad,
          grU_good,grU_bad,//NOTE: ADDED 5/17/23
          grT_good,grT_bad,
    ...
    def muN, muF
    def varN, varF 
    def totN, totF, totA, totU, totLT, aveLT //NOTE: ADDED 5/17/23. REMOVED: , totT
    def totFacc = sectors.collect{0}
    def reluncN, reluncF
    def NF,NFerrH,NFerrL,LT
    def valN,valF,valU,valA //NOTE: ADDED 5/17/23
    def defectList = []
    def badfile
    inList.each { obj ->
      if(obj.contains("/grA_") && (pid!=0 ? obj.contains("PID${pid}") : true)) {
        ...
        if( hasSectors || (!hasSectors && sector==0)) {
          ...
          grU = inTdir.getObject("/${runnum}/grU_${det}_${runnum}") //NOTE: ADDED 5/17/23 ... //NOTE: check formatting from qaPlot.groovy
          grT = inTdir.getObject("/${runnum}/grT_${det}_${runnum}")
          listA = graph2list(grA)
          listN = graph2list(grN)
          listF = graph2list(grF)
          listU = graph2list(grU) //NOTE: ADDED 5/17/23
          ...
          totN = listN.sum()
          totF = listF.sum()
          totU = listU.sum() //NOTE: ADDED 5/17/23 (AND MOVED FROM BELOW-2). REMOVED: = 0
          totA = totN/totF
          
          // compute livetime
          totLT = totF /totU // from total FC charge
          aveLT = listT.size()>0 ? listT.sum() / listT.size() : 0 // average livetime for the run //NOTE: ADDED 5/17/23. REMOVED the FOLLOWING
          ...
          grA.getDataSize(0).times { i ->
            valN = grN.getDataY(i)
            valF = grF.getDataY(i)
            valU = grU.getDataY(i) //NOTE: ADDED 5/17/23
            grN.setError(i,0,Math.sqrt(valN))
            grF.setError(i,0,Math.sqrt(valF))
            grU.setError(i,0,Math.sqrt(valU)) //NOTE: ADDED 5/17/23
            grA.setError(i,0,
              (valN/valF) * Math.sqrt(
          ...
          (grU_good,grU_bad) = splitGraph(grU) //NOTE: ADDED 5/17/23
          ...
          grA.getDataSize(0).times { i -> 
            ...
            if(badfile) {
              ...
              copyPoint(grU,grU_bad,i)//NOTE: ADDED 5/17/23
              ...
              addEpochPlotPoint(epochPlotTree[sector][epoch]['grU_bad'],grU,i,runnum)//NOTE: ADDED 5/17/23
              ...
              copyPoint(grU,grU_good,i)//NOTE: ADDED 5/17/23
              ...
              addEpochPlotPoint(epochPlotTree[sector][epoch]['grU_good'],grU,i,runnum)//NOTE: ADDED 5/17/23
          ...
          addGraphsToHipo(outHipoU)//NOTE: ADDED 5/17/23
          addGraphsToHipo(outHipoF)
          addGraphsToHipo(outHipoFA)//NOTE: ADDED 5/17/23
          addGraphsToHipo(outHipoLTT)//NOTE: ADDED 5/17/23
          addGraphsToHipo(outHipoLTA)//NOTE: ADDEDD 5/17/23. REMOVED: addGraphsToHipo(outHipoT)
    ...
    if(hasSectors) {
      writeTimeline(outHipoU,TLU,"faraday_cup_charge_ungated",true)//NOTE: ADDED 5/17/23
      writeTimeline(outHipoF,TLF,"faraday_cup_charge_gated",true)//NOTE: ADDED 5/17/23
      writeTimeline(outHipoFA,TLFA,"faraday_cup_charge_gated_accumulated",true)//NOTE: ADDED 5/17/23
      writeTimeline(outHipoLTT,TLTT,"live_time_from_fc_charge_totals",true)//NOTE: ADDED 5/17/23
      writeTimeline(outHipoLTA,TLTA,"live_time_average",true)//NOTE: ADDED 5/17/23. REMOVED:  THE FOLLOWING:
    ...
    //NOTE: ADDED 5/17/23: COMMENTED OUT: writeTimeline(outHipoRhoNF,TLrhoNF,"faraday_cup_vs_${particleN}_yield_correlation",true)
